...
function GradientCSS(a) {
    ...
    a && (this.backgroundColor = a.color || "rgba(0, 0, 0, 0)", this.format = a.format || "rgba", this.order = a.order || ["0"], this.priority = a.priority || 0, this.gradientIndex = a.css, (b = _importCssCode(a.css[this.priority].bg_image)) ? (this.linearOrientation = 0 === b.linearOrientation ? 0 : b.linearOrientation || "to bottom", this.radialShape = b.radialShape || "ellipse", this.radialSize =
        b.radialSize || "farthest-corner", this.radialHorizontalSize = b.radialHorizontalSize || 50, this.radialHorizontalSizeUnit = b.radialHorizontalSizeUnit || "em", this.radialVerticalSize = b.radialVerticalSize || 50, this.radialVerticalSizeUnit = b.radialVerticalSizeUnit || "em", this.radialVerticalPosition = 0 === b.radialVerticalPosition ? 0 : b.radialVerticalPosition || "center", this.radialHorizontalPosition = 0 === b.radialHorizontalPosition ? 0 : b.radialHorizontalPosition || "center", this.radialVerticalPositionUnit = b.radialVerticalPositionUnit ||
        "key-value", this.radialHorizontalPositionUnit = b.radialHorizontalPositionUnit || "key-value", this.conicStartAngle = b.conicStartAngle || 0, this.conicHorizontalPosition = 0 === b.conicHorizontalPosition ? 0 : b.conicHorizontalPosition || "center", this.conicVerticalPosition = 0 === b.conicVerticalPosition ? 0 : b.conicVerticalPosition || "center", this.conicHorizontalPositionUnit = b.conicHorizontalPositionUnit || "px", this.conicVerticalPositionUnit = b.conicVerticalPositionUnit || "px", this.type = b.type || "linear", this.repeat = b.repeat ||
        !1, this.colorStops = _generateColorStops(b, this.widthDefault), this.changeFormatColor(this.format)) : a.css && (this.parse = !1))
}
GradientCSS.prototype.addStopMarker = function(a, b, c) {
    ...
    for (e = 0; e < f.length; e++) f[e] === b ? (console.log("moreen"), console.log(b), f[e].htmlBlock = _createStopMarker(e, b, this.widthDefault), f[e].htmlBlock.addClass("selected")) : f[e].location >= b.location ? f[e].htmlBlock ? (f[e].htmlBlock.removeClass("selected"), f[e].htmlBlock.attr("position", f[e].location), f[e].htmlBlock.attr("imarker", e)) : f[e].htmlBlock = _createStopMarker(e,
        b, this.widthDefault) : f[e].htmlBlock.removeClass("selected");
    if (c) {
        c = f.length;
        a = [];
        b = [];
...
GradientCSS.prototype.updateGradientPreview = function() {
    ...
    if (this.colorStopHSL) { console.log("kkkkkkk"); var b = this.getCssCode() + getPoints(this.colorStopHSL[this.priority], this.format) } else b = this.getCssCode() + getPoints(this.colorStops, this.format), a.points = this.colorStops;
    a.bg_image = b;
...
GradientCSS.prototype.showFormatColor = function(a) {
    ...
    console.log(c);
    c = c.substring(2);
    console.log(CSS.supports("background-image", c));
    return c
};
...
GradientCSS.prototype.getCssCode = function() {
    ...
    var c = this.repeat;
    if ("linear" === b) {
        var d = this.linearOrientation;
        a += "linear-gradient(" + d;
        console.log(a);
        a = "number" === typeof d ? a + "deg," : a + ","
    } else if ("radial" === b) {
        b = this.radialShape;
        var e = this.radialHorizontalPosition,
            f = this.radialVerticalPosition,
            h = this.radialHorizontalPositionUnit,
            g = this.radialVerticalPositionUnit;
        "number" === typeof e && (e += h);
        "number" === typeof f && (f += g);
        "ellipse" === b ? (console.log(d), console.log(gradient), d = this.radialSize.match(/closest-side|closest-corner|farthest-side|farthest-corner/) ?
            this.radialSize : this.radialHorizontalSize + this.radialHorizontalSizeUnit + " " + this.radialVerticalSize + this.radialVerticalSizeUnit) : d = this.radialSize.match(/closest-side|closest-corner|farthest-side|farthest-corner/) ? this.radialSize : this.radialHorizontalSize + this.radialHorizontalSizeUnit;
        a += "radial-gradient(" + this.radialShape + " " + d + " at " + e + " " + f + ","
    } else "conic" === b && (d = this.conicStartAngle, b = this.conicHorizontalPosition, e = this.conicHorizontalPositionUnit, f = this.conicVerticalPosition, h = this.conicVerticalPositionUnit,
        "number" === typeof b && (b += e), "number" === typeof f && (f += h), a += "conic-gradient(from " + d + "deg at " + b + " " + f + ",");
    c && (a = "repeating-" + a);
    return this.gradientIndex[this.priority].pattern = a
};

function getPoints(a, b) { console.log(a); var c, d = a.length; var e = " " + _displayColorStop(a[0].color, a[0].location, b); for (c = 1; c < d; c++) e += ", " + _displayColorStop(a[c].color, a[c].location, b); return e + ")" }
...
function _dragAndDrop(a, b, c) {
    ...
    d > e ? (console.log("greater"), a.addStopMarker(a.colorStops, b, !0)) : (console.log("lower"), a.addStopMarker(a.colorStops, b, !1));
...
function _refreshLocation(a, b, c) {
    ...
    console.log(a);
    console.log(d);
    b > parseFloat(e) ? (console.log("greater"), c.addStopMarker(gradient.colorStops, d, !0)) : (console.log("smaller"), c.addStopMarker(gradient.colorStops, d, !1));
...
function _calculatePosition(a, b) {
    console.log(a);
    console.log(b);
    var c = Math.round(a / b * 100) - 1;
    return 0 > c ? 0 : 100 < c ? 100 : c
}
...
function _createStopMarker(a, b, c) {
    ...
    f.append(d);
    console.log(e.displayColor("rgba"));
...
function _importAllGradientsFromLocalStorage() {
    ...
    a || (a = [{ v: " linear-gradient(315deg, rgba(92,46,145,1) 0%, rgba(172,84,160,1) 100%)", w: " 8px 8px", x: " 0px 0px", y: " repeat", z: "rgba(0,0,0,0)" }, {
            ...
            z: "rgba(0,173,255,1)"
        }, { v: " linear-gradient(315deg, rgba(157,32,82,1) 25%, rgba(0,0,0,0) 25%, rgba(0,0,0,0) 50%, rgba(157,32,82,1) 50%, rgba(157,32,82,1) 75%, rgba(0,0,0,0) 75%), linear-gradient(45deg, rgba(17,22,39,1) 30%, rgba(86,26,60,1) 30%, rgba(86,26,60,1) 50%, rgba(17,22,39,1) 50%, rgba(17,22,39,1) 80%, rgba(86,26,60,1) 80%)", w: " 48px 48px,  32px 32px", x: " 10px 10px,  0px 0px", y: " repeat,  repeat", z: "rgba(127,89,108,1)" },
        {
        ...
        { v: " radial-gradient(ellipse farthest-corner at 50% 50%, rgba(255,255,255,1) 20%, rgba(0,0,0,0) 20%), linear-gradient( rgba(247,148,29,1) 0%, rgba(242,88,34,1) 100%)", w: " 12px 12px,  auto", x: " 0px 0px,  0% 0%", y: " repeat,  repeat", z: "rgba(0,0,0,0)" }, {
            ...
            y: "repeat, repeat",
            z: "rgba(119,85,51,1)"
        }, { v: "radial-gradient(ellipse farthest-corner at center center, rgba(206,222,171,1) 12%,rgba(0,0,0,0) 12%,rgba(0,0,0,0) 88%,rgba(206,222,171,1) 88%), linear-gradient( rgba(53,46,118,1) 0%,rgba(0,123,255,1) 50%,rgba(89,0,255,1) 100%)", w: "25px 25px, auto", x: "0% 0%, 0% 0%", y: "repeat, repeat", z: "rgba(119,85,51,1)" }
    ], a = JSON.stringify(a), localStorage.setItem("swatches16", a));
...
function _importCssCode(a) {
    ...
    console.log(a);
    a.match("linear-gradient") ? (a = a.slice(a.search("linear-gradient") + 15), b = _importLinear(a, c), console.log(b)) : a.match("radial-gradient") ? (a = a.slice(a.search("radial-gradient") + 15), b = _importRadial(a, c)) : a.match("conic-gradient") && (a = a.slice(a.search("conic-gradient") + 14), b = _importConic(a, c));
...
function _importCssCodeGetListPoints(a) {
    ...
    for (a = a.replace(/ /gi, "");;) {
        ...
        else if (b = a.match("transparent")) d.color = new UserColor({
            format: "rgba",
            color: {
                r: 0,
                g: 0,
...
function G(a, b, c, d, e, f) {
    ...
    console.log(c);
    console.log(d);
    console.log(CSS.supports("background-image", b));
    this.id = a;
    var h = {};
...
G.prototype.filterSize = function(a) {
    ...
    a = a.replace(/\s/g, "");
    console.log(a);
    if (b = a.match(/contain|cover/)) { console.log(b); var e = b[0]; var f = b[0]; var h = c = "" } else if (b = a.match(/((-?\d{1,3})(px|em|rem|%)|(auto))(((-?\d{1,3})(px|em|rem|%))|(auto))?/)) console.log(b), b[4] ? (e = b[4], f = b[4]) : b[2] && b[3] && (e = parseFloat(b[2]), c = b[3], f = parseFloat(b[2]), h = b[3]), b[5] && (b[6] && (f = parseFloat(b[7]), h = b[8]), b[9] && (f = b[9], h = ""));
    d.bg_size_value1 = e;
    d.bg_size_value2 = f;
    d.bg_size_s1 = e;
    d.bg_size_s2 = f;
...
G.prototype.createHide = function(a, b) {
    ...
    d.change(function() {
        ...
        d.is(":checked") ? (console.log("checked"), h.visible = !1, $(".grad__css-container").remove(), $(".grad__view-css").removeClass("is-checked"), b.addClass("freeze"), e.spinner1.spinner("disable"), e.spinner2.spinner("disable"), f.spinner1.spinner("disable"), f.spinner2.spinner("disable")) : (h.visible = !0, b.removeClass("freeze"),
            e.spinner1.spinner("enable"), e.spinner2.spinner("enable"), f.spinner1.spinner("enable"), f.spinner2.spinner("enable"));
...
G.prototype.createRemove = function(a) {
    ...
    c.click(function() {
        ...
        if (b === gradient.priority) {
            ...
            if (0 === d && 0 === e) { console.log("llll"); return }
            d === e ? (console.log("max"), d = parseFloat(gradient.order[d - 1]), e = gradient.gradientIndex[d].bg_elem, e.addClass("grad--selected"), d = _secLoadGrad(d), d.parse &&
                (gradient = d, _newGrad())) : d < e && (console.log("greater"), d = parseFloat(gradient.order[d + 1]), e = gradient.gradientIndex[d].bg_elem, e.addClass("grad--selected"), d = _secLoadGrad(d), d.parse && (gradient = d, _newGrad()))
        }
        a.remove();
        _updateContainer()
    });
...
function firstLoad() {
    ...
    a && 0 < a.length && (a = unescape(a), a = a.replace(/\\/g, ""), a = a.replace(/'/g, ""), a = a.substr(1), $(".current").attr("style", a), console.log(a));
...
$(".stop-markers").click(function(a) {
    if (addAMark) {
        var b;
        gradient.saveHSLStops();
        var c = _calculatePosition(a.clientX - (this.offsetParent.offsetLeft + this.offsetLeft), gradient.widthDefault);
        (b = $("#color-value-button").spectrum("get").toRgbString()) ? (b = parseColor(b), console.log(b)) : b = gradient.colorStops[0].color.clone();
        console.log(c);
        gradient.addStopMarker(gradient.colorStops, { color: b, location: c });
        gradient.updateGradientEditor();
        gradient.updateGradientPreview();
        gradient.showAllColorStops();
        _updateGradientCSS(["bg_image"]);
...
$("#report-bug-btn").click(function() { window.open("file:///home/ko_ko/Downloads/gradient-editor/gradient-editor/contact.html") });
PREVIEW.resizable({
    resize: function(a, b) {
        var c = $(this).width(),
            d = $(this).height();
...
$(window).onerror = function(a, b, c, d, e) { e && console.log(e.stack) };
$("#format").click(function() {
    ...
    var c = _updateGradientCSS(["bg_image", "bg_size", "bg_position", "bg_repeat", "bg_color"]);
    $("#hsla-format").css("background-size", c.bg_size);
    $("#hsla-format").css("background-position", c.bg_position);
    $("#hsla-format").css("background-repeat", c.bg_repeat);
    $("#hsla-format").css("background-color",
        c.bg_color);
    $("#hex-format").css("background-size", c.bg_size);
    $("#hex-format").css("background-position", c.bg_position);
    $("#hex-format").css("background-repeat", c.bg_repeat);
    $("#hex-format").css("background-color", c.bg_color);
    console.log(a);
    console.log(b);
    console.log(CSS.supports("background-image", a));
    console.log(CSS.supports("background-image", b))
});
...
function _refreshLocationMini(a, b) {
    ...
    c.location = b;
    c.color = e;
    b > parseFloat(d) ? (console.log(">"), newgrad.addStopMarker(newgrad.colorStops, c, !0)) : newgrad.addStopMarker(newgrad.colorStops, c, !1);
...
function updateInfoMini(a) {
    ...
    d && (c = a.displayColor("rgba"), console.log(c), console.log(a), d.color = a, d.htmlBlock.attr("color", c), b.children().css("background-color", c), newgrad.updateGradient())
}
...
$("#mini-show").click(function() { console.log(newgrad) });
...
$("#add-gradient__color-value-button").spectrum({
    ...
    show: function() {
        var a = _getActiveElementMini(),
            b = a.offset().left - 285;
        a = a.offset().top + 52;
        var c = $("#add-gradient__color-value-button").spectrum("container");
        c.css("top", a + "px");
        c.css("left", b + "px");
        gradient.saveHSLStops()
    }
